...
cp hooks/termux-notify.conf.example ~/.claude/hooks/termux-notify.conf
# 必要に応じて REMOTE_HOST 等を編集
vim ~/.claude/hooks/termux-notify.conf
```

設定項目の詳細は [`hooks/termux-notify.conf.example`](hooks/termux-notify.conf.example) を参照。

#### 2. 通知スクリプトを配置

```bash
cp hooks/termux-notify.sh ~/.claude/hooks/termux-notify.sh
chmod +x ~/.claude/hooks/termux-notify.sh
```

スクリプトの全文は [`hooks/termux-notify.sh`](hooks/termux-notify.sh) を参照。
